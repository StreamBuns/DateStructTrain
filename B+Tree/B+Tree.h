...
*/
void insertKey(BPTreeNode* p,int j,int k,BPTreeNode* ap){
    int i,x=p->n,beMaxKey=p->key[p->n];
    for(i=0;i<(p->n)-j;i++){
        p->key[x+1]=p->key[x];
        p->ptr[x+1]=p->ptr[x];
        x--;
...
bool BPTree::Insert(const int&x){
    ...
    if(p->n<m){
        ...
        insertKey(p, j, k, ap);
            ...
            insertKey(p, j, k, ap);
//            while(p->parent->key[p->parent->n]!=p->key[p->n])i++;//定位到父节点相应的与之前p中最大值相等的位置，换成现在p中的最大值
            p->parent->key[p->parent->n]=k;
    ...
    k=q->key[q->n];     //k存储q当前的最大的key
    ap=q;
    if(p->parent!=NULL){
        t=p->parent;
        i=j=0;
        
        while(t->key[i+1]==k1)j++;
        t->key[i+1]=p->key[s+1];
        //找到与之前p的最大key值相等的key，并且替换成现在p中最大的值
        while(t->key[j+1]<k)j++;
        //找到现在cv的q的最大key值要插入的地方
        q->parent=p->parent;
        p=t;
    }
    else{
